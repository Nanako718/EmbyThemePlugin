...
namespace Emby.Plugin.MistyTheme
{
    public static class ThemeManager
    {
        ...
        public static void Inject(string webPath)
        {
            ...
            if (html.Contains(HeadStart)) return;

            var backup = indexPath + ".misty-bak";
            if (!File.Exists(backup)) File.Copy(indexPath, backup);

            // 把字体复制到 dashboard-ui 根目录，直接用 /AgibotDisplay-Regular.woff2 访问
            CopyFont(webPath);
        ...
        {
            var s = html.IndexOf(start, System.StringComparison.Ordinal);
            var e = html.IndexOf(end,   System.StringComparison.Ordinal);
            if (s < 0 || e < 0) return html;
            return html.Remove(s, e + end.Length - s);
        }

        private static void CopyFont(string webPath)
        {
            var target = Path.Combine(webPath, "AgibotDisplay-Regular.woff2");
            if (File.Exists(target)) return;
            using var stream = ReadResource("AgibotDisplay-Regular.woff2");
        ...
        private static Stream? ReadResource(string filename)
        {
            ...
            return asm.GetManifestResourceStream($"{ns}.web.{filename}");
        }

        private static string ReadTextResource(string filename)
        {
            using var stream = ReadResource(filename);
            if (stream == null) return "";
        ...
        {
            var css = ReadTextResource("styles.css");
            return $@"<link rel=""preload"" href=""/AgibotDisplay-Regular.woff2"" as=""font"" type=""font/woff2"" crossorigin>
<style>
    @font-face {{
        font-family: 'Agibot Display';
        src: url('/AgibotDisplay-Regular.woff2') format('woff2');
        font-weight: 400;
        font-style: normal;
...
{css}
        ...
        {
            var js = ReadTextResource("main.js");
            return $@"<div class=""misty-loading"">
